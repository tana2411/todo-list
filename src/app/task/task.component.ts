...
openDetail(index: number) {
 ...
 removeTask(id: number) {
  ...
  if (this.search.trim()) {
    ...
    this.currenttask$.next(filtered);
  }
}
  ...
  toggleCheck(id: number) {
    ...
    this.checked$.next(current.includes(id)
...
remove() {
  ...
  const updatedTasks = this.tasks$.value.filter(task => !checkedIds.includes(task.id));
  this.tasks$.next(updatedTasks);
  localStorage.setItem('tasks', JSON.stringify(updatedTasks));
  this.checked$.next([]);
  ...
  searchTask() {
    ...
    if (!query) {   
      ...
      console.log('cr',this.currenttask$.value)
